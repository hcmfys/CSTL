...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl='urn:schemas-microsoft-com:xslt'
    ...
    <xsl:template match="projects">
        <xsl:for-each select="project">
            ...
            <h2>
                <xsl:if test="@solution"><a _locID="Solution">Solution</a>:
                    <xsl:value-of select="@solution"/>
                </xsl:if>
                <xsl:if test="not(@solution)"><a _locID="Project">Project</a>:
            ...
            <table cellpadding="2" cellspacing="0" width="98%" border="1" bordercolor="white" class="infotable">
                ...
                <xsl:for-each select="source">
                    ...
                    <xsl:if test="count(event)!=count(event[@error-level='4'])">
                        ...
                        <tr class="row">
                            <td class="content">
                                <A HREF="javascript:">
                                    <xsl:attribute name="onClick">javascript:document.images['<xsl:value-of
                                            select="$source-id"/>'].click()
                                    </xsl:attribute>
                                    <IMG border="0" _locID="IMG.alt" _locAttrData="alt" alt="expand/collapse section"
                                         class="expandable" height="11" onclick="changepic()"
                                         src="_UpgradeReport_Files/UpgradeReport_Plus.gif" width="9">
                                        <xsl:attribute name="name">
                                            <xsl:value-of select="$source-id"/>
                                        </xsl:attribute>
                                        <xsl:attribute name="child">src<xsl:value-of select="$source-id"/>
                                        </xsl:attribute>
                                    </IMG>
                        ...
                        <tr class="collapsed" bgcolor="#ffffff">
                            <xsl:attribute name="id">src<xsl:value-of select="$source-id"/>
                            </xsl:attribute>

                            <td colspan="7">
                                <table width="97%" border="1" bordercolor="#dcdcdc" rules="cols" class="issuetable">
                                    <tr>
                                        <td colspan="7" class="issuetitle" _locID="ConversionIssues">Conversion Report -
                                            <xsl:value-of select="@name"/>:
                                        </td>
                                    </tr>
                ...
                <tr valign="top">
                    ...
                    <td class="foot">
                        <a _locID="Converted3">Converted</a>:&#32;<xsl:value-of
                            select="count(source/event[@error-level='3' and @description='Converted'])"/>
                        <BR/>
                        <a _locID="NotConverted">Not converted</a>:&#32;<xsl:value-of
                            select="count(source) - count(source/event[@error-level='3' and @description='Converted'])"/>
                    </td>
                    <td class="foot">
    ...
    <xsl:template match="UpgradeLog">
        <html>
            ...
            <body topmargin="0" leftmargin="0" rightmargin="0" onclick="outliner();">
                ...
                <p>
                    <span class="note">
                        <b _locID="TimeOfConversion">Time of Conversion:</b>&#32;&#32;<xsl:value-of
                            select="Properties/Property[@Name='Date']/@Value"/>&#32;&#32;<xsl:value-of
                            select="Properties/Property[@Name='Time']/@Value"/>
                        <br/>
                    </span>
                </p>
